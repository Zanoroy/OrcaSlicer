...
{
	let searchInput = $("#DiscoverySearchInput");
	if (searchInput.length === 0)
		return;

	let keyword = (searchInput.val() || "").toString().trim();
	if (keyword.length === 0) {
		OnClickModelDepot();
		return;
	}
	...
	tSend['url']="https://makerworld.com/en/search/models?keyword=" + encodeURIComponent(keyword);

	SendWXMessage( JSON.stringify(tSend) );
}

function OnOpenMakerlab()
{
	var tSend={};
	tSend['sequence_id']=Math.round(new Date() / 1000);
	tSend['command']="common_openurl";
	tSend['url']="https://makerworld.com/en/makerlab";
...
$(document).on('keydown', '#DiscoverySearchInput', function(event) {
	if (event.key === 'Enter') {
